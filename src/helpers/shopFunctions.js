...
const xablau = [];
let xabliu = 0;
const xablau2 = (xablauTheThrid) => {
  xabliu += xablauTheThrid;
  priceHtml.innerHTML = xabliu.toFixed(2);
};
...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  xablau2(price);
  const alteraTotal = () => {
    const soma = xablau.reduce((acc, curr) => acc + curr, 0);
    return soma;
  };
  alteraTotal();
  const finalValue = alteraTotal();
  priceHtml.innerHTML = finalValue;
